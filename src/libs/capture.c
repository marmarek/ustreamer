...
int us_capture_open(us_capture_s *cap) {
	...
	if (cap->dv_timings && cap->media_path) {
		if ((run->media_fd = open(cap->media_path, O_RDWR | O_NONBLOCK)) < 0) {
...
static int _capture_find_and_open_v4l_subdev(const us_capture_s *cap, us_media_pad_s media_pads[3]) {
	...
	if (us_xioctl(run->media_fd, MEDIA_IOC_DEVICE_INFO, &dev_info) < 0) {
	...
	if (us_xioctl(run->media_fd, MEDIA_IOC_G_TOPOLOGY, &topology_info) < 0) {
		_LOG_PERROR("Failed to query topology info");
		return -1;
	}

	struct media_v2_entity entities[topology_info.num_entities]; // cppcheck-suppress constParameterPointer
	struct media_v2_link links[topology_info.num_links]; // cppcheck-suppress constParameterPointer
	struct media_v2_interface interfaces[topology_info.num_interfaces]; // cppcheck-suppress constParameterPointer
	struct media_v2_pad pads[topology_info.num_pads]; // cppcheck-suppress constParameterPointer
	...
	if (!(source_link_flags & MEDIA_LNK_FL_ENABLED)) {
		if (_capture_media_configure_link(run->media_fd,
